...
class AlertMonitorService {
  ...
  void _evaluate(AcState state) {
    ...
    if (_settings.weatherVsSetpointEnabled && !_weatherVsSetpointFired) {
      ...
      if (outdoor < state.temperature) {
        ...
        _notifications.show(
          id: 400,
          title: 'Ngoài trời mát hơn setpoint',
          body:
              'Ngoài trời ${outdoor.toStringAsFixed(1)}°C < setpoint ${state.temperature}°C ($source). Cân nhắc tắt máy lạnh.',
        );
      }
    }
  }
  ...
  Future<void> triggerWeatherAlert(int setpoint) async {
    ...
    final outdoor = _outdoorWeather?.temperatureC ?? simulatedOutdoorTemp;
    final source =
        (_outdoorWeather?.isSimulated ?? true) ? 'giả lập' : 'Open-Meteo';
    await _notifications.show(
      id: 400,
      title: 'Thời tiết ngoài trời',
      body:
          'Ngoài trời ${outdoor.toStringAsFixed(1)}°C, setpoint $setpoint°C ($source).',
    );
  }
